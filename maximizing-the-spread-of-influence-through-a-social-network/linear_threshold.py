...
def generate_graph(n: int = 100, m: int = 2) -> nx.DiGraph: # This is here just incase/running independently
...
def run_lt(G: nx.DiGraph, seeds: List[int]) -> Set[int]:
    active = set(seeds)
    thresholds = {}
...
def greedy_select(G: nx.DiGraph, k: int, simulations: float) -> List[int]:
    selected = set()
...
def run_simulation(
    n: int = 100, m: int = 2, k: int = 5, simulations: int = 1000, G: nx.DiGraph = None
):
    if G is None:
